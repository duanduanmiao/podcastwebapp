...
def test_read_csv_file(csv_reader):
    podcasts = csv_reader.read_csv_file(TEST_DATA_PATH / 'episodes.csv')
    count = 0
...
def test_load_episodes(csv_reader):
    episodes = list(csv_reader.read_csv_file(TEST_DATA_PATH / 'episodes.csv'))
    assert len(episodes) == 3

    episode = episodes[0]
    assert episode[0] == '1'
    assert episode[4] == '2739'
    assert episode[6] == "2017-12-01 00:09:47+00"


def test_load_podcasts(csv_reader):
    podcasts = list(csv_reader.read_csv_file(TEST_DATA_PATH / 'podcasts.csv'))

    assert len(podcasts) == 3
    podcast = podcasts[0]
    assert podcast[0] == '1'
    assert podcast[4] == "English"


def test_populate(csv_reader):
    episodes = list(csv_reader.read_csv_file(TEST_DATA_PATH / 'episodes.csv'))
    podcasts = list(csv_reader.read_csv_file(TEST_DATA_PATH / 'podcasts.csv'))
...
def test_episode_initialization():
    episode = Episode(id=1, podcast=1, title="Test Episode", link="http://testlink.com", length=60,
                      description="Test Description", date="2023-08-19")

    assert episode.id == 1
    assert episode.podcast == 1
    assert episode.title == "Test Episode"
    assert episode.link == "http://testlink.com"
    assert episode.length == 60
    assert episode.description == "Test Description"
    assert episode.date == "2023-08-19"


def test_episode_repr():
    episode = Episode(id=1, podcast="Podcast1", title="Test Episode", link="http://testlink.com", length=60,
                      description="Test Description", date="2023-08-19")

    assert repr(episode) == "<Episode 1: 'Test Episode'>"


def test_episode_hash():
    episode1 = Episode(id=1, podcast="Podcast1", title="Test Episode", link="http://testlink.com", length=60,
                       description="Test Description", date="2023-08-19")
    episode2 = Episode(id=2, podcast="Podcast1", title="Another Episode", link="http://anotherlink.com", length=45,
                       description="Another Description", date="2023-09-01")

    assert hash(episode1) != hash(episode2)


def test_review_initialization():
    review = Review(review_id=1, podcast_id="1", user="User1", title="test", rating=5, content="Great podcast!")
    assert review.id == 1
    assert review.title == "test"
    assert review.rating == 5
    assert review.content == "Great podcast!"


def test_review_repr():
    review = Review(review_id=1, podcast_id="Podcast1", user="User1", title="test", rating=5, content="Great podcast!")
    assert repr(review) == "<Review 1: 'User1'>"


def test_playlist_initialization():
    playlist = Playlist(playlist_id=1, user=User(1, "username", "password"))
    assert playlist.id == 1
    assert playlist.user.username == "username"
    assert playlist.podcasts == list()
    assert playlist.episodes == list()


def test_playlist_eq():
    playlist1 = Playlist(playlist_id=1, user="User1")
    playlist2 = Playlist(playlist_id=2, user="User2")

    assert playlist1 != playlist2

    playlist3 = Playlist(playlist_id=1, user="User3")
...
def test_playlist_hash():
    ...
    playlist3 = Playlist(playlist_id=1, user="User3")
    assert hash(playlist1) == hash(playlist3)


def test_playlist_add_episode():
    playlist = Playlist(playlist_id=1, user="User1")
    episode = Episode(id=1, podcast=1, title="Test Episode", link="http://testlink.com", length=60,
                      description="Test Description", date="2023-08-19")
